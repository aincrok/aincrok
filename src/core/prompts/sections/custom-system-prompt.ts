...
export function getSystemPromptFilePath(cwd: string, mode: Mode): string {
	// kilocode_change
	return path.join(cwd, ".aincrok", `system-prompt-${mode}`)
}

/**
 * Loads custom system prompt from a file at .aincrok/system-prompt-[mode slug]
...
/**
 * Ensures the .aincrok directory exists, creating it if necessary
 */
export async function ensureRooDirectory(cwd: string): Promise<void> {
	// kilocode_change
	const rooDir = path.join(cwd, ".aincrok")
