...
Workflows automate repetitive tasks by defining step-by-step instructions for Aincrok to execute. Invoke any workflow by typing `/[workflow-name.md]` in the chat.

<img src="/docs/img/slash-commands/workflows.png" alt="Workflows tab in Aincrok" width="600" />

## Creating Workflows

Workflows are markdown files stored in `.aincrok/workflows/`:
- **Global workflows**: `~/.aincrok/workflows/` (available in all projects)
- **Project workflows**: `[project]/.aincrok/workflows/` (project-specific)
...
Create a file called `submit-pr.md` in your `.aincrok/workflows` directory:
...
Now you can trigger this workflow by typing `/submit-pr.md` in the chat. Aincrok will:
