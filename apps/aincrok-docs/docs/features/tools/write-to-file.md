...
- When Aincrok needs to create a new file from scratch
- When Aincrok needs to completely rewrite an existing file
...
- Access Control: Validates against `.aincrokignore` restrictions before making changes
...
1. **Parameter Validation**: Validates the required parameters and permissions
    ...
    - Validates the file is allowed (not restricted by `.aincrokignore`)
...
- When creating a new project, Aincrok generates multiple files but lets you review each before committing changes.
- When setting up configuration files, Aincrok shows the proposed configuration in a diff view for approval.
- When generating documentation, Aincrok creates markdown files but lets you make final adjustments in the diff view.
- When developing a prototype, Aincrok shows complete source files in a diff view where you can fine-tune before saving.
