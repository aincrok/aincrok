---
title: Using MCP in Aincrok
sidebar_label: Using MCP in Aincrok
---

# Using MCP in Aincrok

Model Context Protocol (MCP) extends Aincrok's capabilities by connecting to external tools and services. This guide covers everything you need to know about using MCP with Aincrok.

<YouTubeEmbed
  url="https://youtu.be/6O9RQoQRX8A"
  caption="Demostrating MCP installation in Aincrok"
/>
...
2.  **Project-level Configuration**: Defined in a `.aincrok/mcp.json` file within your project's root directory. This allows you to set up project-specific servers and share configurations with your team by committing the file to version control. Aincrok automatically detects and loads this file if it exists.
...
You can edit both global and project-level MCP configuration files directly from the Aincrok MCP settings view.

1. Click the <Codicon name="gear" /> icon in the top navigation of the Aincrok pane to open `Settings`.
...
4. Click the appropriate button:
  ...
  <img src="/img/using-mcp-in-aincrok/mcp-installed-config.png" alt="Edit Global MCP and Edit Project MCP buttons" width="600" />
...
````json
{
  ...
  ```
  *Example of MCP Server config in Aincrok (STDIO Transport)*
...
2. Press the `Delete` button on the confirmation box
  ...
  <img src="/img/using-mcp-in-aincrok/using-mcp-in-aincrok-5.png" alt="Delete confirmation box" width="400" />
...
<img src="/img/using-mcp-in-aincrok/using-mcp-in-aincrok-6.png" alt="Network Timeout pulldown" width="400" />
...
<img src="/img/using-mcp-in-aincrok/using-mcp-in-aincrok-7.png" alt="Always allow checkbox for MCP tools" width="120" />
...
Aincrok does not come with any pre-installed MCP servers. You'll need to find and install them separately.

- **Community Repositories:** Check for community-maintained lists of MCP servers on GitHub
- **Ask Aincrok:** You can ask Aincrok to help you find or even create MCP servers
- **Build Your Own:** Create custom MCP servers using the SDK to extend Aincrok with your own tools

For full SDK documentation, visit the [MCP GitHub repository](https://github.com/modelcontextprotocol/).

## Using MCP Tools in Your Workflow

After configuring an MCP server, Aincrok will automatically detect available tools and resources. To use them:

1. Type your request in the Aincrok chat interface
2. Aincrok will identify when an MCP tool can help with your task
...
- **Permission Errors:** Ensure proper API keys and credentials are configured in your `mcp_settings.json` (for global settings) or `.aincrok/mcp.json` (for project settings).
