---
title: 在Aincrok中使用MCP
sidebar_label: 在Aincrok中使用MCP
---

# 在Aincrok中使用MCP

模型上下文协议（MCP）通过连接外部工具和服务扩展了Aincrok的功能。本指南涵盖了在Aincrok中使用MCP所需了解的所有内容。

<YouTubeEmbed
  url="https://youtu.be/6O9RQoQRX8A"
  caption="演示Aincrok中的MCP安装"
/>
...
2. **项目级配置**：在项目根目录的`.aincrok/mcp.json`文件中定义。这允许你设置项目特定的服务器，并通过将文件提交到版本控制与团队共享配置。如果存在，Aincrok会自动检测并加载此文件。

**优先级**：如果服务器名称同时存在于全局和项目配置中，则**项目级配置优先**。

### 编辑MCP设置文件

你可以直接从Aincrok MCP设置视图编辑全局和项目级MCP配置文件：

1.  单击 Aincrok 窗格顶部导航栏中的 <Codicon name="gear" /> 图标以打开 `Settings`。
...
_Aincrok中的MCP服务器配置示例（STDIO传输）_
...
Aincrok不附带任何预安装的MCP服务器。你需要单独查找并安装它们。

- **社区仓库**：在GitHub上查看社区维护的MCP服务器列表
- **询问Aincrok**：你可以请Aincrok帮助你查找甚至创建MCP服务器（当"[启用MCP服务器创建](#enabling-or-disabling-mcp-server-creation)"启用时）
- **自行构建**：使用SDK创建自定义MCP服务器，使用你自己的工具扩展Aincrok

有关完整的SDK文档，请访问[MCP GitHub仓库](https://github.com/modelcontextprotocol/)。

## 在工作流中使用MCP工具

配置MCP服务器后，Aincrok将自动检测可用的工具和资源。要使用它们：

1. 在Aincrok聊天界面中输入你的请求
2. Aincrok将识别何时MCP工具可以帮助完成任务
...
- **权限错误**：确保在`mcp_settings.json`（用于全局设置）或`.aincrok/mcp.json`（用于项目设置）中配置了正确的API密钥和凭据。
